...
function new_target() {
  player.arrows = [];
  if (shoot == 0 && turn > -1 && wait == 0)
    // DID NOT SHOOT OR MOVE DURING PREVIOUS TARGET
    draw_score();
  shoot = 0;
  wait = 0;
  moved = 0;
...
function updateTable(r, data) {
  ...
  var x = document.getElementById('tg-PdFaH').rows[parseInt(r, 10)].cells;
  x[parseInt(1, 10)].innerHTML = tmp;
  if (data[2] == 1) tmp = 'X';
  else tmp = '-';
  var x = document.getElementById('tg-PdFaH').rows[parseInt(r, 10)].cells;
  x[parseInt(2, 10)].innerHTML = tmp;
  if (data[3] == 1) tmp = 'X';
  else tmp = '-';
  var x = document.getElementById('tg-PdFaH').rows[parseInt(r, 10)].cells;
  x[parseInt(3, 10)].innerHTML = tmp;
  if (data[4] == 1) tmp = 'X';
  else tmp = '-';
  var x = document.getElementById('tg-PdFaH').rows[parseInt(r, 10)].cells;
  x[parseInt(4, 10)].innerHTML = tmp;
  if (data[5] == 1) tmp = 'slow';
  else if (data[5] == 2) tmp = 'medium';
  else tmp = 'fast';
  var x = document.getElementById('tg-PdFaH').rows[parseInt(r, 10)].cells;
  x[parseInt(5, 10)].innerHTML = tmp;
  if (data[6] == 1) tmp = 'three (3)';
  else if (data[6] == 2) tmp = 'five (5)';
  else tmp = 'seven (7)';
  var x = document.getElementById('tg-PdFaH').rows[parseInt(r, 10)].cells;
  x[parseInt(6, 10)].innerHTML = tmp;
  var x = document.getElementById('tg-PdFaH').rows[parseInt(r, 10)].cells;
  x[parseInt(7, 10)].innerHTML = data[7] + '/20';
  var x = document.getElementById('tg-PdFaH').rows[parseInt(r, 10)].cells;
  x[parseInt(8, 10)].innerHTML = data[8];
}
...
function init() {
  ...
  player = {
    ...
    draw: function () {
      ...
      if (TARGETS.color[turn] == 1 || rules[3] == 0)
        // green uniform
        pen.drawImage(GplayerImage, player.x, player.y, player.w, player.h);
      // red uniform
      else pen.drawImage(RplayerImage, player.x, player.y, player.w, player.h);
    },
    ...
    shoot: function () {
      if (counter - prev_counter >= 0 && player.arrows.length == 0) {
        ...
        } else var b = new arrow(this.x + this.w / 2);
...
function isInvalidMove() {
  if (rules[0] == 1 && moved == 1 && EXPECTED.shouldMove[turn] == 0) return 1;
  else return 0;
}
...
function arrow(x) {
  ...
  this.draw = function () {
    if (this.state == 'active') pen.drawImage(arrowImage, this.x, this.y, this.w, this.h);
  };
...
function estimate_scores(user, all_responses, movement) {
  ...
  cpoints =
    (s[0] * speed_factor * diff_factor +
      s[1] * speed_factor * diff_factor +
      s[2] * speed_factor * diff_factor +
      s[3] * speed_factor * diff_factor) /
    4.0;
  cpoints = Math.round(cpoints * 10) / 10;
  if (practice == 0) scores[5] += cpoints;
  return [
    s[0] * speed_factor * diff_factor,
    s[1] * speed_factor * diff_factor,
    s[2] * speed_factor * diff_factor,
